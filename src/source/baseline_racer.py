...
def compute_gae(next_value, rewards, masks, values, gamma=GAMMA, lam=GAE_LAMBDA):
    ...
    gae = 0
    returns = []
    print("size of all related ->rewards masks values ",len(rewards),len(masks),len(values),"\n")
    for step in reversed(range(len(rewards))):
        print("This",step,"step in compue_gae. rewards[step]",rewards[step]," values[step+1]",values[step+1]," masks[step]",masks[step]," values[step]",values[step],"\n")
        delta = rewards[step] + gamma * values[step + 1] * masks[step] - values[step]
        print("delta is :",delta,"\n")
        gae = delta + gamma * lam * masks[step] * gae
        print("this is gae value : ",gae,"\n")
        # prepend to get correct order back
        print("Shape in1 returns = ",len(returns))
        returns.insert(0, gae + values[step])
        print("Shape in2 returns = ",len(returns))

    print("Shape of returns = ",len(returns))
    return returns
...
while not early_stop:
    ...
    for stepNumber in range(PPO_STEPS):
       ...
       # linear_vel_object_temp_numpy = np.array([linear_vel_object_temp.x_val,linear_vel_object_temp.y_val,linear_vel_object_temp.z_val])
        ...
        state = next_state
        frame_idx += 1
    print("Hi , i finally quitted from loop \n")
    next_state = torch.FloatTensor(next_state).to(device)
    _, next_value = model(next_state)
    returns = compute_gae(next_value, rewards, masks, values)
    print("This is returns function after exiting compueage len,data",len(returns),returns,"\n")
    ...
    print("Printing Returns Matrix and size is ",returns.shape,"\n")
    print(returns,"\n")
    print("Printing Values Matrix and size is ",values.shape,"\n")
    print(values,"\n")
