...
class BaselineRacer(object):
    def __init__(self, drone_name = "drone_1", viz_traj=True, viz_traj_color_rgba=[1.0, 0.0, 0.0, 1.0], viz_image_cv2=True):
        ...
        self.current_angular_acceleration = None
    ...
    def odometry_callback(self):
        ...
        # in world frame:
        self.current_position = self.airsim_client_odom.getMultirotorState().kinematics_estimated.position
        self.current_linear_velocity = self.airsim_client_odom.getMultirotorState().kinematics_estimated.linear_velocity
        self.current_angular_velocity = self.airsim_client_odom.getMultirotorState().kinematics_estimated.angular_velocity
        self.current_linear_acceleration = self.airsim_client_odom.getMultirotorState().kinematics_estimated.linear_acceleration
        self.current_angular_acceleration = self.airsim_client_odom.getMultirotorState().kinematics_estimated.angular_acceleration
        self.current_enemy = self.airsim_client_odom.simGetGroundTruthKinematics("drone_2")
